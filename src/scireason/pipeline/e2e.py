...
try:  # local offline RU->EN translator; no-op if unavailable.
    from .translate import translate_en
except Exception:  # pragma: no cover - defensive import
    translate_en = None  # type: ignore[assignment]
...
def _to_english_query(query: str) -> str:
    """Translate a Russian query to English for English-language paper APIs.

    Best-effort and offline-safe: returns the input unchanged if no local
    translator is available or the text is already English.
    """

    if translate_en is None or not (query or "").strip():
        return query
    try:
        return translate_en(query)
    except Exception:
        return query
...
def run_pipeline(
    *,
    ...
    console.print(f"[bold cyan]Search[/bold cyan] query='{search_query}' sources={sources or ['all']}")
    papers = search_papers(search_query, sources=sources, limit=search_limit)
    papers = _rank_papers(papers, search_query)
    ...
    if generate_report:
        ...
        try:
            ...
            artifacts = write_run_report(
                ...
                papers=selected,
                graph_json_path=out / "temporal_kg.json",
                records_by_id={r.paper_id: r for r in paper_records},
                use_llm=use_llm_for_hypotheses,
            )
            console.print(f"[green]Report:[/green] {artifacts['report']}")
        except Exception as e:
            console.print(
                f"[yellow]Report generation failed ({type(e).__name__}: {e}); continuing without it.[/yellow]"
            )
