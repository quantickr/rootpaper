...
class Settings(BaseSettings):
    ...
    # Имя бота (без @) для Telegram Login Widget на сайте.
    telegram_bot_username: str | None = None

    # ===== Хранилище (общее для бота и сайта) =====
    # Если задан DATABASE_URL (postgresql://...), используется PostgreSQL,
    # иначе — SQLite-файл (офлайн-режим по умолчанию).
    database_url: str | None = None
    sqlite_db_path: str | None = None  # переопределить путь к SQLite-файлу

    # ===== Веб-сайт =====
    # Секрет для подписи cookie-сессий. В проде ОБЯЗАТЕЛЬНО задать свой.
    web_secret_key: str = "change-me-in-production"
    web_host: str = "0.0.0.0"
    web_port: int = 8080
    # Публичный базовый URL сайта (для ссылок в письмах). Без завершающего "/".
    web_base_url: str = "http://localhost:8080"
    # Cookie только по HTTPS (включать в проде за TLS-прокси).
    web_session_https_only: bool = False
    # Время жизни токена подтверждения email (секунды).
    email_verify_ttl_seconds: int = 86400
    # Email(ы) администраторов (CSV). Такие пользователи получают роль admin
    # при регистрации.
    admin_emails: str = ""

    # ===== SMTP (подтверждение email) =====
    # По умолчанию — пустой хост: письма не отправляются, токен пишется в лог
    # (dev-fallback). Задай smtp_host, чтобы включить реальную отправку.
    smtp_host: str | None = None
    smtp_port: int = 465
    smtp_user: str | None = None
    smtp_password: str | None = None
    # "ssl" (порт 465) | "starttls" (587) | "none" (25/2525).
    smtp_security: str = "ssl"
    smtp_from: str | None = None  # адрес отправителя (по умолчанию smtp_user)
    smtp_from_name: str = "top-papers-graph"
