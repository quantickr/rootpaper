...
def run_in_sandbox(
    ...
    *,
    ...
    # Support the smolagents idiom `final_answer(<value>)` in addition to the
    # variable-assignment idiom `final_answer = <value>`. The callable records the
    # provided value in a dedicated slot so we can recover it after execution.
    _final_answer_slot: Dict[str, Any] = {}

    def _final_answer(value: Any = None) -> Any:
        _final_answer_slot["value"] = value
        return value

    env["final_answer"] = _final_answer
    ...
    # If the code called `final_answer(<value>)`, promote the captured value so the
    # caller can read `env["final_answer"]` uniformly. If the code instead rebound
    # `final_answer` to a plain value (assignment idiom), leave that value as-is.
    if "value" in _final_answer_slot and env.get("final_answer") is _final_answer:
        env["final_answer"] = _final_answer_slot["value"]

    if "final_answer" not in env or env["final_answer"] is _final_answer:
    ...
    def run(self, task: str, *, context: Optional[str] = None) -> Any:
        ...
        for step in range(1, self.max_steps + 1):
            user = (
                ...
                + "2) Return your result via `final_answer`. Either call `final_answer(<value>)` "
                + "or assign `final_answer = <value>`; both work. The value must be JSON-serializable.\n"
